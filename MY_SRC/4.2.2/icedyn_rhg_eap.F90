...
CONTAINS
   ...
   SUBROUTINE ice_dyn_rhg_eap( kt, Kmm, pstress1_i, pstress2_i, pstress12_i, pshear_i, pdivu_i, pdelta_i, paniso_11, paniso_12, prdg_conv )
      ...
      !
      REAL(wp) ::   factorTau                                           ! rho0 * drag_io
      ...
      DO jter = 1 , nn_nevp                           !    loop over jter    !
        ...
        ! Save beta at T-points for further computations
         ...
         IF( MOD(jter,2) == 0 ) THEN ! even iterations
            ...
            DO_2D( 0, 0, 0, 0 )
               !                 !--- tau_io/(v_oce - v_ice)
               factorTau = rho0 * ( drag_io(ji,jj) + drag_io(ji+1,jj)) * 0.5
               zTauO = zaV(ji,jj) * factorTau * SQRT( ( v_ice (ji,jj) - v_oce (ji,jj) ) * ( v_ice (ji,jj) - v_oce (ji,jj) )  &
...
!!          CALL agrif_interp_ice( 'V', jter, nn_nevp )
            ...
            DO_2D( 0, 0, 0, 0 )
               !                 !--- tau_io/(u_oce - u_ice)
               factorTau = rho0 * ( drag_io(ji,jj) + drag_io(ji+1,jj)) * 0.5
               zTauO = zaU(ji,jj) * factorTau * SQRT( ( u_ice (ji,jj) - u_oce (ji,jj) ) * ( u_ice (ji,jj) - u_oce (ji,jj) )  &
