MODULE sbcblk
   ...
   REAL(wp), PUBLIC ::   rn_Cd_i          !: 
   REAL(wp), PUBLIC ::   rn_Ce_i          !:
   REAL(wp), PUBLIC ::   rn_Ch_i          !:
   ...
   INTEGER , PUBLIC ::   nn_frm            !: = 1 : affects momentum and heat transfer coefficient 
                                          ...
                                          !(default)
                                          !: = 2 : affects only momentum
                                          !transfer coefficient 
                                          !:       for ocean-ice and atmos-ice
                                          !: = 3 : affect momentum and heat
   ...
   REAL(wp), PUBLIC ::   crw              ! local form drag coefficient [0,1]
   REAL(wp), PUBLIC ::   cfa              ! floe edge atm [0,1]
   REAL(wp), PUBLIC ::   cfw              ! floe edge ocean [0,1]
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namsbc_blk in configuration namelist' )
      ...
      IF(lwp) THEN                     !** Control print
         ...
         CASE( np_ice_frm )    
            ...
            WRITE(numout,*) '                         with csw    = ', REAL(csw,4)
            WRITE(numout,*) '                         with csa    = ', REAL(csa,4)
            WRITE(numout,*) '                         with cra    = ', REAL(cra,4)
            WRITE(numout,*) '                         with crw    = ', REAL(crw,4)
            WRITE(numout,*) '                         with cfa    = ', REAL(cfa,4)
            WRITE(numout,*) '                         with cfw    = ', REAL(cfw,4) 
         END SELECT
#endif
         !#LB.
         !
