...
#  include "do_loop_substitute.h90"
   ...
   !! $OriginalId: icevar.F90 15385 2021-10-15 13:52:48Z clem $
...
CONTAINS
   ...
   SUBROUTINE ice_var_agg( kn )
      ...
      at_ip_eff(:,:) = SUM( a_ip_eff(:,:,:)*a_i(:,:,:), dim=3 ) ! melt ponds
...
!!                a criteria for icy area (i.e. a_i > epsi20 and v_i > epsi20 )
      ...
      WHERE    ( h_il(:,:,:) <= rn_pnd_hl_min )  ;   a_ip_eff(:,:,:) = a_ip_frac(:,:,:)       ! lid is very thin.  Expose all the pond
      ELSEWHERE( h_il(:,:,:) >= rn_pnd_hl_max )  ;   a_ip_eff(:,:,:) = 0._wp                  ! lid is very thick. Cover all the pond up with ice and snow
      ELSEWHERE                                  ;   a_ip_eff(:,:,:) = a_ip_frac(:,:,:) * &   ! lid is in between. Expose part of the pond
         &                                                             ( rn_pnd_hl_max - h_il(:,:,:) ) / ( rn_pnd_hl_max - rn_pnd_hl_min )
      END WHERE
      IF ( ln_pnd_ae_snw ) THEN
         WHERE( h_ip(:,:,:) < h_s(:,:,:) )   a_ip_eff(:,:,:) = 0._wp
      ENDIF
      !
